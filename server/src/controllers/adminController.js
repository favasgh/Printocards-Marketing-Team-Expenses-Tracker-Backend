...
export const getDailyExpenses = asyncHandler(async (req, res) => {
  ...
  const expenses = await Expense.find({
    ...
    .sort({ 'userId.name': 1, createdAt: -1 })
    .lean();

  const grouped = new Map();
  const overallSummary = buildSummaryTemplate();

  expenses.forEach((expense) => {
    const salesmanId = expense.userId?._id?.toString() || 'unknown';

    if (!grouped.has(salesmanId)) {
      grouped.set(salesmanId, {
        salesman: {
          _id: expense.userId?._id || null,
          name: expense.userId?.name || 'Unknown',
          email: expense.userId?.email || '',
        },
        expenses: [],
        summary: buildSummaryTemplate(),
      });
    }

    const group = grouped.get(salesmanId);
    group.expenses.push(expense);
    group.summary.totalAmount += expense.amount || 0;
    group.summary.totalExpenses += 1;

    const statusKey = expense.status?.toLowerCase();
    if (statusKey && typeof group.summary[statusKey] === 'number') {
      group.summary[statusKey] += 1;
    }
  });

  const salesmen = Array.from(grouped.values()).map((group) => {
    Object.keys(overallSummary).forEach((key) => {
      overallSummary[key] += group.summary[key];
    });
    return group;
  });
